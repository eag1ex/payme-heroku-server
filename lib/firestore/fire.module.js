...
/**
 * firebase rules: https://www.securecoding.com/blog/cloud-firestore-security-policy/
 */
/**
 *
...
class Firestore extends Fireconfig {
  ...
  INIT_FIRE_NINJA() {
    try {
      ...
      // this.DB.listCollections().then((n) => {
      //   console.log("listCollections", n);
      // });
    } catch (err) {
  ...
  deleteDocuments(idsArr, col = "", done) {
    ...
    collection
      ...
      .then((snapshot) => {
        snapshot.forEach((doc) => {
          const found = idsArr.filter(
            (n) => Number(n) === doc.data().id
          ).length;
          if (found) {
            dels.push(doc.id);
          }
      ...
      .then((delRefs) => {
        if (!delRefs.length)
          return done({ error: true, message: `no items found to delete` });
        const deleted = [];
        ...
        Promise.all(deleted)
          .then((d) => {
            //   console.log('???', d)
              ...
              success: true,
              message: `deleted count:${d.length}`,
              response: true,
            });
          })
          .catch((err) => {
  ...
  setNewDynamicDocument(col = "", data, done) {
    if (!this.dbHash[col]) {
      notify.s("[setNewDynamicDocument] docID/col not set", true);
        ...
        error: true,
        message: "[setNewDynamicDocument] docID/col not set",
      });
      return;
    }
